...
public class MyRecursiveNode implements IRecursiveNode {
	...
	public void execRecQueryOptMaterialized(Dataset ds, Query recursiveQuery, String recursiveURI) {
		...
		if (recursive_query.length == 1) {
			...
			while (true) {
				...
				while (aux.hasNext()) {
					Triple tr = aux.next();
					dsgRecursive.add(NodeFactory.createURI("http://example.org/recursiveTemp"), tr.getSubject(),
							tr.getPredicate(), tr.getObject());
				}
				dsg.addGraph(NodeFactory.createURI(recursiveURI),
						dsgRecursive.getGraph(NodeFactory.createURI("http://example.org/recursiveTemp")));
				dsgRecursive.removeGraph(NodeFactory.createURI("http://example.org/recursiveTemp"));
			...
			while (baseModel.hasNext()) {
				...
				dsgRecursive.add(NodeFactory.createURI("http://example.org/recursiveTemp"), tr.getSubject(),
						tr.getPredicate(), tr.getObject());
				dsgRecursive.add(NodeFactory.createURI("http://example.org/recursive"), tr.getSubject(),
						tr.getPredicate(), tr.getObject());
			}
			dsg.addGraph(NodeFactory.createURI(recursiveURI),
					dsgRecursive.getGraph(NodeFactory.createURI("http://example.org/recursiveTemp")));
			...
			dsg.removeGraph(NodeFactory.createURI(recursiveURI));
			dsg.addGraph(NodeFactory.createURI(recursiveURI),
					dsgRecursive.getGraph(NodeFactory.createURI("http://example.org/recursive")));
			qexecBase.close();
			dsgRecursive.close();
		}
	...
	public void execRecQueryOptMaterializedLim(Dataset ds, Query recursiveQuery, String recursiveURI,
			int limit) {
		Query[] rec_query = parseOpt(recursiveQuery);
		DatasetGraph dsg = ds.asDatasetGraph();
		dsg.removeGraph(NodeFactory.createURI(recursiveURI));
