    	...
    	if(query.isRecursive)
    	{
    		for(int i = 0; i< query.ConstructRecursiveQueries.size();i++)
    		{
    			if (ExecRecursive.RECURSIVE_METHOD.equalsIgnoreCase("MEM-OPT")) {
    				RecursiveNode.execRecQueryOpt(dataset, (query.ConstructRecursiveQueries).get(i), (query.ConstructRecursiveQueries).get(i).recursiveURI);
    			}
    			else if (ExecRecursive.RECURSIVE_METHOD.equalsIgnoreCase("DISK-OPT")) {
        			RecursiveNode.execRecQueryOptMaterialized(dataset, (query.ConstructRecursiveQueries).get(i), (query.ConstructRecursiveQueries).get(i).recursiveURI);
    			}
    			else if (ExecRecursive.RECURSIVE_METHOD.equalsIgnoreCase("MEM-OPT-LIMIT")) {
        			RecursiveNode.execRecQueryOptLim(dataset, (query.ConstructRecursiveQueries).get(i), (query.ConstructRecursiveQueries).get(i).recursiveURI, ExecRecursive.NUMBER_OF_ITERATIONS);
    			}
    			else if (ExecRecursive.RECURSIVE_METHOD.equalsIgnoreCase("DISK-OPT-LIMIT")) {
        			RecursiveNode.execRecQueryOptMaterializedLim(dataset, (query.ConstructRecursiveQueries).get(i), (query.ConstructRecursiveQueries).get(i).recursiveURI, ExecRecursive.NUMBER_OF_ITERATIONS);
    			}
    			else
    			{
        			RecursiveNode.execRecQueryWithoutUnion(dataset, (query.ConstructRecursiveQueries).get(i), (query.ConstructRecursiveQueries).get(i).recursiveURI);
    			}
