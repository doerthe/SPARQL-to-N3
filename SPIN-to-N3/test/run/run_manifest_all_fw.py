...
def run_manifest_all_fw(path, test, check_compl, engine):
    ...
    for mf in g.subjects(RDF.type, MF.Manifest):
        ...
        if lst is not None:
            for el in Collection(g=g, list=lst):
                path = str(el)
                run_manifest_all_fw(path, test, check_compl, engine)
        # test entries
...
def run_test(g, test, compl, engine):   
    ...
    if compl:
        ...
        subprocess.run(['./test_select_all_fw.sh', query, data, engine, ordered])
    logger.info("")
...
if __name__ == '__main__':
    ...
    if compl is None and engine is None:
        ...
        run_manifest_all_fw(path, test, compl, engine)