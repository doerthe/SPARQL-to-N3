...
time_gen_n3=$( TIMEFORMAT="%R"; { time { eye $spin_file ../../auxiliary-files/aux_iso.n3 --query ../../queries/query_general.n3 --nope --quantify https://eyereasoner.github.io/.well-known/genid/ > $n3_file 2>$n3_stderr; } } 2>&1 )
if [[ $verbose == "true" ]]; then
...
if [[ $recursion == "true" ]]; then
    ...
    time_exec_n3=$( TIMEFORMAT="%R"; { time { eye ../../auxiliary-files/runtime_iso.n3 $n3_file --turtle $nt_file --pass-only-new --nope > $result_file 2>/dev/null; } } 2>&1 )
else
    # not including query as data (before):
    time_exec_n3=$( TIMEFORMAT="%R"; { time { eye ../../auxiliary-files/runtime_iso.n3 --turtle $nt_file --query $n3_file --nope > $result_file 2>/dev/null; } } 2>&1 )
fi
